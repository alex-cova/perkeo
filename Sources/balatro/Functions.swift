...
class Functions : Lock {
    ...
    func randchoice<T: Item>(_ ID : String, _ items : [T]) -> T {
        ...
        if (!params.showman && isLocked(item) || ("RETRY" == item.rawValue || "RETRY2" == item.rawValue)) {
            var resample = 2
            while (true) {
                item = items[randint("\(ID)_resample\(resample)", 0, items.count - 1)]
                resample += 1
                if ((!isLocked(item) && ("RETRY" != item.rawValue || "RETRY2" != item.rawValue)) || resample > 1000){
                    return item
                }
            }
        }
    ...
    func nextTarot(_ source : String,_ ante : Int,_ soulable : Bool) -> Item {
        if (soulable && (params.showman || !isLocked(Specials.THE_SHOUL)) && random(Functions.soul_TarotArr[ante]) > 0.997) {
    ...
    func  nextPlanet(_ source : String,_ ante : Int,_ soulable : Bool) -> Item {
        if (soulable && (params.showman || !isLocked(Specials.BLACKHOLE)) && random(Functions.soul_PlanetArr[ante]) > 0.997) {
    ...
    func nextJoker(_ source : String, joker1Arr : [String], joker2Arr : [String], joker3Arr : [String], joker4Arr : [String],
                   rarityArr : [String], editionArr : [String], _  ante : Int,_  hasStickers : Bool) -> JokerData{
        // Get rarity
        var rarity = ""
        ...
        case "4":
            ...
            else{ joker = randchoice(joker4Arr[ante], Functions.LEGENDARY_JOKERS)}
        ...
        if (hasStickers) {
            if (params.version > 10103) {
                ...
                if (params.stake == Stake.Black_Stake || params.stake == Stake.Blue_Stake || params.stake == Stake.Purple_Stake ||
    ...
    func nextShopItem(_ ante : Int) -> ShopItem{
        ...
        switch (type) {
        case .Joker:
            let jkr = nextJoker("sho", joker1Arr:  Functions.joker1ShoArr,
    ...
    func nextBuffoonPack(_ size : Int,_ ante : Int) -> [JokerData]{
        ...
        for i in 0..<size {
            pack.append(nextJoker("buf",joker1Arr:  Functions.joker1BufArr,
                                  joker2Arr: Functions.joker2BufArr,
                                  joker3Arr: Functions.joker3BufArr,
                                  joker4Arr: Functions.joker4BufArr,
                                  rarityArr: Functions.rarityBufArr, editionArr: Functions.editionShoArr,  ante, true))
            if (!params.showman){
                lock(pack[i].joker)
            }
        }
    ...
    static let stake_shop_joker_eternalArr : [String] = construct(pattern: "stake_shop_joker_eternal%d")
